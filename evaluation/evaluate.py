...
def evaluate(pred_dir, method, testset, only_S_MAE=False, epoch=0):
    ...
    with open(filename, 'a+') as file_to_write:
        ...
        biou_max, biou_mean = biou['curve'].max(), biou['curve'].mean()
        tb.add_row(
            [
                ...
                mba.round(3), biou_max.round(3), biou_mean.round(3),
            ] if not only_S_MAE else [method, testset, mae.round(3), sm.round(3)]
        )
        print(tb)
        file_to_write.write(str(tb).replace('+', '|')+'\n')
        file_to_write.close()
    return {'e_max': e_max, 'e_mean': e_mean, 'e_adp': e_adp, 'sm': sm, 'mae': mae, 'f_max': f_max, 'f_mean': f_mean, 'f_wfm': f_wfm, 'f_adp': f_adp, 'hce': hce, 'mba': mba, 'biou_max': biou_max, 'biou_mean': biou_mean}
